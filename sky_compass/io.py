...
def json_io(s : str) -> str:
  ...
  if "sections" in output_dat.keys():
...
def mapping_result_to_dict(result: MappingResult):
  # TODO: Do we NEED this succeeded field? We could get away with using "result" (result is not None) as a conditional.
  return {"succeeded": result.succeeded, "result": result.result, "min": result.min, "max": result.max}
...
def deserialized_io(raw, skip_verification : bool = False):
  ...
  if not skip_verification:
    if region is None: return output
