...
def json_io(s: str) -> str:
    ...
    if output_dat["response"] != "Invalid input":
        output_dat.update(
            {"sections": [str(section) for section in output_dat["sections"]]}
        )  # dumps doesn't have a way to map GlobalSection, so we map it ourselves
...
def deserialized_io(raw, skip_verification: bool = False):
    ...
    if not skip_verification:
        if not isinstance(raw, dict):
            return output  # raw is not a dict (huh?)
        if raw.keys() != ["address", "region", "section"]:
            return output
        if not isinstance(raw["address"], int):
            return output
        if not isinstance(raw["region"], str):
            return output
        if not isinstance(raw["section"], str):
            return output
    address = raw["address"]
    region = region_from_str(raw["region"])
    if (not skip_verification) and (not region):
        return output
    ...
    if len(sections) != 1 or Issue.INVALID_SECTION in section_issues:
        ...
        )  # A section issue prevents mapping from continuing. (This is normal; an expected use of sky-compass is just to check the section of an address.)
        return output
    # We're good to map!
    ...
    output.update(
        {
            "response": "Success",
            "outputs": {
                "na": mapping_result_to_dict(na_result),
                "eu": mapping_result_to_dict(eu_result),
                "jp": mapping_result_to_dict(jp_result),
            },
        }
    )
    return output
